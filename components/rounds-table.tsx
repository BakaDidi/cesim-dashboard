  ...
  useEffect(() => {
    const fetchRounds = async () => {
      try {
        ...
        const basicRounds = await response.json()

        // Récupérer les données détaillées pour chaque tour
        const detailedRounds = await Promise.all(
            basicRounds.map(async (round: Round) => {
              // Récupérer les performances de l'équipe "Corpo'mate" (votre équipe)
              const perfResponse = await fetch(`/api/performances?roundId=${round.id}&equipe=Corpo'mate`)
              const performances = await perfResponse.json()

              // Récupérer les parts de marché
              const marketResponse = await fetch(`/api/market-shares?roundId=${round.id}&equipe=Corpo'mate`)
              const marketShares = await marketResponse.json()

              return {
                ...round,
                performance: performances.length > 0 ? performances[0] : null,
                marketShare: marketShares.length > 0 ? marketShares[0] : null
              }
            })
        )

        setRounds(detailedRounds)
      } catch (error) {
